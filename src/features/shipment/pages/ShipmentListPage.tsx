...
export function ShipmentListPage() {
  ...
  const mappedData = useMemo(() => {
    ...
    if (!customerList || !Array.isArray(customerList)) {
      return data.data.map(s => ({ ...s, customerName: "Cargando..." }));
    }
    
    return data.data.map(shipment => ({
      ...shipment,
      customerName: customerList.find((c: any) => c.id === shipment.customer_id)?.name || "Cliente no encontrado"
    }));
  }, [data, customers]);
  ...
  const columns: Column<any>[] = [
    {
      header: "Guía",
      accessor: (item) => (
        <span className="font-black text-primary tracking-tighter">#{item.guide_number}</span>
      ),
    },
    {
      header: "Cliente",
    ...
    },
    {
      ...
      accessor: (item) => (
        <div className="flex items-center gap-2">
          ...
          />
          <span className="text-xs font-bold uppercase">{item.shipping_type === "LAND" ? "Terrestre" : "Marítimo"}</span>
        </div>
      ),
    },
    {
    ...
    },
    {
      header: "Total",
      accessor: (item) => (
        <span className="font-bold text-on-surface">${(item.total_price + item.applied_extra_fee).toLocaleString()}</span>
      ),
    },
  ];
      ...
      <div className={cn(
        ...
        <div className="p-6 border-bottom border-outline-variant/10 flex items-center justify-between bg-surface-container-low/50">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-black uppercase tracking-widest text-on-surface">Filtros</h3>
              <p className="text-[10px] text-outline font-bold">AJUSTES AVANZADOS</p>
            </div>
          </div>
          <button 
            onClick={() => setIsFiltersOpen(false)}
    ...
    <div className="flex items-center gap-2">
      ...
      <button 
        ...
        className={cn(
          ...
          (customerId || dispatchLocation || shippingType || shippingStatus || dateFrom || dateTo) 
    ...
    <div className="relative">
      ...
      <div className="space-y-6">
        <DataTable
          ...
          onAdd={() => onOpen("CREATE_SHIPMENT")}
          addLabel="Nuevo Envío"
          searchKey="customerName"
          searchPlaceholder="Buscar por cliente..."
          filters={tableFilters}
          onView={(item) => onOpen("VIEW_SHIPMENT", item)}
          onEdit={(item) => {
            if (item.shipping_status === "PENDING") {
              onOpen("EDIT_SHIPMENT", item);
            } else {
              toast.error("Solo se pueden editar envíos en estado Pendiente");
            }
          }}
          onDelete={(item) => {
            if (item.shipping_status === "PENDING") {
              onOpen("DELETE_SHIPMENT", item);
            } else {
              toast.error("No se puede eliminar un envío que ya ha sido procesado");
            }
          }}
        />
      </div>
