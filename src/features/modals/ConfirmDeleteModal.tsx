...
export function ConfirmDeleteModal({ data }: ConfirmDeleteModalProps) {
  const queryClient = useQueryClient();
  const { onClose } = useModalStore();

  const mutation = useMutation({
    mutationFn: () => customerService.deleteCustomer(data.id),
    onSuccess: () => {
      ...
      <div className="flex flex-col items-center text-center space-y-4">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-on-surface-variant font-medium leading-relaxed">
            Estás a punto de eliminar a <span className="font-bold text-error">{data.name}</span>. 
            Esta acción es irreversible y podría afectar otros registros vinculados.
          </p>
        </div>
      </div>
